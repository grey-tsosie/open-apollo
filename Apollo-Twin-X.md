...
The Twin X DUO has identification and raw audio input/output verified on Linux,
with 10 playback channels, 16
...
`scripts/dev-module.sh claim` uses `tools/apollo-init.sh --no-daemon`.
The initializer loads the module, then diagnoses the DSP before firmware replay.
A stalled or frozen DSP stops initialization unless `claim --force` is supplied.
Failed or invalid health reads and firmware replay failures stop initialization
even with `--force`. The helper sets `APOLLO_SKIP_PIPEWIRE=1`, so claiming does
not restart audio services, select profiles, or change the default output.
Manual ownership assumes no installed module or autoload configuration; the
helper warns when it finds either or cannot complete an inspection. The default
installer enables DKMS.

The project rule is "NEVER rmmod ua_apollo": full-mode removal can kill the
Thunderbolt link on an x4 and require a cold boot. `trace` and `release` refuse
by default. An intentional experiment requires `--allow-unsafe-unload`.
`probe` only unloads its own identification-only load.

`twinx_dsp=1` is an untested, default-off experiment for device type `0x23` only.
It does not enable x4-derived programs on Gen 2. Gen 2 retains its previous
8 playback / 8 capture / 2 preamp / 2 Hi-Z placeholders pending measurements.
