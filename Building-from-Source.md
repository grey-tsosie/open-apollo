...
For the standard quick-start process (build, load, verify), see the [Installation](Installation) guide.
...
| `make firmware-install` | Install `ua-apollo-mixer.bin` (from the repo root) and `configs/firmware/ua-apollo-plugin-chain.bin` to `/lib/firmware/`. The second blob feeds the plugin chain replay, see [Experimental Features](Experimental-Features) |
