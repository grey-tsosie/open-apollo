...
These commands are sent in batches of 64 to DSP 0, with the ring reprogrammed between batches.

The chain is **replayed by default** after every successful ACEFACE connect (once per boot; replug and reconnect do not re-run it). The Windows capture contains host physical addresses in its `DMA_REF` entries, so the driver loads matching payload buffers from `/lib/firmware/ua-apollo-plugin-chain.bin` via `request_firmware()` and substitutes the Linux `dma_addr` of each buffer before sending. The purpose is to load the DSP programs that forward preamp flag writes (PAD, 48V, Mic/Line) from SRAM to the ARM MCU; without them those relays do not toggle. If the firmware blob is missing, the `DMA_REF` entries are skipped and the driver logs a warning.

Two module parameters control it:

| Parameter | Default | Effect |
|-----------|---------|--------|
| `no_plugins` | `false` | Skip the chain entirely (ACEFACE connect still runs) |
| `skip_bus_coeff` | `true` | Skip the 745 `BUS_COEFF` entries. Sending them has caused sequence-counter deadlocks; the correct encoding for that command is still unknown, so fader/pan/send coefficients are not sent from Linux |

This mechanism is a captured-snapshot replay with hand-substituted addresses, not a reconstruction of the real init sequence. Treat it as experimental; see [Experimental Features](Experimental-Features). It is also the foundation for any future UAD DSP plugin (UADx) support.
...
- [Register Map](Register-Map) — complete BAR0 register layout
- [Architecture Overview](Architecture-Overview) — system diagram and hardware write paths
