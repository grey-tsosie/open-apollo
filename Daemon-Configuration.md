...
Capture the model's control tree from a working UA Mixer Engine (see [Device Capture (macOS)](Device-Capture-macOS)), then flatten it into the daemon's format:
...
- [Daemon Setup](Daemon-Setup) — getting started with the daemon
- [Protocol Reference](Protocol-Reference) — protocol wire formats
