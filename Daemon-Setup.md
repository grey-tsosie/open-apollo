...
```bash
# Optional (enable the WebSocket server on 4721, used by UA Connect)
pip install websockets

# Optional (enable Bonjour auto-discovery)
pip install zeroconf

# Optional (enable ALSA input metering)
pip install pyalsaaudio
```

All three are optional. The daemon starts with only the standard library; the WebSocket server, Bonjour, and hardware metering are disabled with a warning when their package is missing. Metering covers input channels only; output meters always report silence.
...
The daemon needs read/write access to `/dev/ua_apollo0`. The installer's udev rule (`configs/udev/91-ua-apollo.rules`) sets the node to mode `0666`, so on a standard install the daemon does not need root; the unit above runs as root only for simplicity.
...
- [Daemon Configuration](Daemon-Configuration) — all command-line flags and options
- [Protocol Reference](Protocol-Reference) — TCP and WebSocket protocol details
- [Architecture Overview](Architecture-Overview) — how the daemon fits into the system
