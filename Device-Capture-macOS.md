...
4. Run DTrace probes for 30 seconds to capture routing data (`--seconds N` changes the duration)
5. Save everything to a JSON file

The output file is saved in the current directory with a timestamp, for example: `apollo-macos-capture-YYYYMMDD-HHMMSS.json`. Use `--output path.json` to choose the location.
...
Add a `devices/apollo-<model>.json` descriptor with the matching `device_type`, and the daemon will select the new map automatically for that model. See [Daemon Configuration](Daemon-Configuration#device-maps) for details.

---

## Next steps

- [Submitting Your Data](Submitting-Data) — how to submit your capture
- [How to Contribute](How-to-Contribute) — other ways to help
