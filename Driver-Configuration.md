...
| Apollo x6 | 24 | 22 | 4 | 2 |
| Apollo x8 | 26 | 26 | 4 | 2 |
| Apollo x8p | 34 | 32 | 8 | 2 |
| Apollo x16 | 34 | 34 | 8 | 2 |

Gen 2 variants share the same channel counts as their base models, except the Apollo x8p Gen 2 (26 / 26).
...
The project includes a WirePlumber rule that sets optimal defaults for Apollo interfaces. It ships in two formats because WirePlumber changed its config language in 0.5:

| WirePlumber | File | Deploy to |
|-------------|------|-----------|
| 0.5 and newer (Fedora 40+, Arch, openSUSE, Debian trixie, Manjaro) | `configs/wireplumber/51-ua-apollo.conf` | `/etc/wireplumber/wireplumber.conf.d/` |
| 0.4.x (Ubuntu 24.04, Debian bookworm) | `configs/wireplumber/51-ua-apollo.lua` | `/etc/wireplumber/main.lua.d/` |

`sudo bash configs/deploy.sh` detects the installed version and copies the right one (the installer runs it for you). To deploy by hand:
...
sudo cp configs/wireplumber/51-ua-apollo.conf /etc/wireplumber/wireplumber.conf.d/
...
A `.lua` file on a 0.5 system is silently ignored, which leaves `api.alsa.disable-mmap` unset and the driver unstable. Both files apply the following properties to all Apollo ALSA nodes:
...
| Apollo S/PDIF In | Audio/Source | Stereo | capture AUX4-5 (untested) |
| Apollo S/PDIF Out | Audio/Sink | Stereo | playback AUX6-7 (untested) |

The two S/PDIF nodes are created but have never been verified with a digital source or receiver attached. See [Experimental Features](Experimental-Features).
...
### Command-line recording and playback

The loopback nodes have stable `node.name` values you can target directly:

```bash
# Record from Mic 1 (mono) and Mic 1+2 (stereo)
...
# Play to the monitor outputs
pw-play --target apollo_monitor /path/to/file.wav

# Watch levels and xruns
pw-top
```

Other targets: `apollo_mic_2`, `apollo_mic_3`, `apollo_mic_4`, `apollo_line_34`, `apollo_line_out_12`, `apollo_line_out_34`.
...
|-------------|------|-------|-------------|
| `Line N Capture Volume` | Integer | -144 to 65 (dB) | Preamp gain |
