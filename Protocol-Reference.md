...
Output meters follow the same path pattern under `/devices/0/outputs/<ch>/meters/`, but they are not implemented: the daemon returns the silence value (-77 dBFS, no clip) for every output channel. Playback metering would need a tap on the PipeWire monitor or a driver ioctl.
...
- [Daemon Setup](Daemon-Setup) — running the daemon
- [Daemon Configuration](Daemon-Configuration) — all command-line flags
