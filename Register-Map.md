...
| Mixer readback | `0x3810`–`0x38B4` | Hardware readback status and 40 data words |
| DSP ring banks (high) | `0x6000`–`0x61FF` | Per-DSP ring buffers (DSP 4–7); base = `0x5E00` + idx × `0x80` with idx 4–7 |
...
ASCII digits 5–8 of the serial string (offset `serial + 4`, not the leading four) identify the device model. For example, an x4 serial of `2019 2005 01xxxx` matches prefix `2005` (device_type `0x1F`). The leading four digits are a separate field: matching on them misidentifies hardware (the x4's leading `2019` is the table's x8p entry). The x8p's own prefix `2017` collides with the x8, which is why that model is pinned by PCI subsystem ID (`0x0014`) instead.
...
The upper 16 bits of each word are the changed-bits mask. If the mask is zero for a word, the DSP ignores that half. See [DSP Protocol](DSP-Protocol) for the sequence counter handshake.

> [!WARNING]
> **Mask semantics and firmware defaults**
>
...
- [DSP Protocol](DSP-Protocol) — ring buffer commands and settings batch protocol
- [Architecture Overview](Architecture-Overview) — system diagram and data flow
