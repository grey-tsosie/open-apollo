...
| [Apollo Twin X (DUO)](Apollo-Twin-X) | 0x23 | 10 | 16 | 2 | 1 | **Identification + raw I/O verified** |
...
| Apollo x8p | 0x20 | 34 | 32 | 8 | 2 | **Partially Verified** |
...
USB Apollo models use UAC 2.0 audio. No kernel module is required — the FX3 firmware is uploaded from userspace on each power-on, and `snd-usb-audio` (with a 4-patch out-of-tree build) handles audio streaming. See [USB RE findings](USB-Apollo-Reverse-Engineering) for protocol details.
...
> [!NOTE]
> USB setup uses `sudo bash scripts/install-usb.sh`. See [Installation](Installation) for details, or [USB RE findings](USB-Apollo-Reverse-Engineering) for protocol details.
...
- Monitor volume, mute, dim, mono. The talkback switch toggles, but talkback audio routing is not implemented (see [Experimental Features](Experimental-Features))
...
- Full duplex at **26 in / 26 out, 48 kHz** — stable over a 60-second soak with zero xruns and no IOMMU faults under `intel_iommu=on`. This test predates the driver fix that raised the x8p to 34 playback / 32 record channels; the additional channels have not been verified on hardware
...
See [USB RE findings](USB-Apollo-Reverse-Engineering) for the full protocol details.
...
See [How to Contribute](How-to-Contribute) for details, or jump straight to the device capture guides:

- [Device Capture (macOS)](Device-Capture-macOS) — capture routing data using DTrace
- [Device Capture (Windows, USB)](Device-Capture-Windows) — capture a USB Apollo's init sequence with USBPcap
