...
**Fix:** The firmware must be obtained from an existing macOS or Windows UAD installation. See [Building from Source — Step 4](Building-from-Source#step-4-install-firmware) for extraction instructions. Then place it at `/lib/firmware/ua-apollo-mixer.bin`.
...
After every kernel update, rebuild the module. Consider setting up [DKMS](Building-from-Source#dkms-setup) for automatic rebuilds.
...
If direct ALSA playback works but PipeWire does not, see the [PipeWire configuration](Driver-Configuration#pipewire-configuration) section.
...
**Fix:** Increase the PipeWire buffer size. Create `~/.config/pipewire/pipewire.conf.d/99-apollo.conf` (nothing creates it for you) with:
...
**Fix:** The firmware binary (`ua-apollo-mixer.bin`) is not included in this repository — it must be obtained from an existing macOS or Windows UAD installation. See [Building from Source — Step 4](Building-from-Source#step-4-install-firmware) for extraction instructions.
...
1. **FX3 firmware not uploaded:** The Cypress FX3 has no onboard flash — firmware must be uploaded every power-on. Check that udev triggered the init script:
   ...
   ```
   The loader is a systemd template unit (`ua-usb-init@<firmware>.service`), so the unit name includes the firmware file. If nothing ran, upload manually with the firmware file for your model (`install-usb.sh` installs the helper as `ua-usb-init`):
   ```bash
   sudo ua-usb-init ApolloSolo.bin     # ApolloTwin.bin or ApolloTwinX.bin for those models
   ```
...
**Fix:** re-run the DSP init. The helper is installed by `install-usb.sh` and runs the copy of `usb-full-init.py` under `/usr/local/lib/ua-usb/`, so the installer must have completed at least once.
```bash
sudo ua-usb-dsp-init
# Then restart PipeWire:
systemctl --user restart pipewire wireplumber
```
